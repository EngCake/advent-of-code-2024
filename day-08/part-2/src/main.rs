...
fn main() {
    ...
    for positions in antennas.values() {
        if positions.len() > 1 {
            antinodes_positions.extend(positions.iter());
        }
        ...
        for (i, &first_position) in positions.iter().enumerate() {
            for &second_position in &positions[i + 1..] {
                ...
                let mut antinode_1 = first_position + delta;
                while map.contains(antinode_1) {
                    antinodes_positions.insert(antinode_1);
                    antinode_1 += delta;
                }

                let mut antinode_2 = second_position - delta;
                while map.contains(antinode_2) {
